...
static irqreturn_t palmas_vbus_irq(int irq, void *_palmas_usb)
{
	...
	struct palmas *palmas = palmas_usb->palmas;
	int status = 0;
	int slave;
...
#if defined(CONFIG_CHARGER_PALMAS) || defined(CONFIG_CHARGER_PALMAS_MODULE)
	/* Inform charger driver of change in vbus state */
	if (palmas->charger)
		palmas_charger_usb_vbus_state(palmas->charger,
...
static int palmas_enable_irq(struct palmas_usb *palmas_usb)
{
	...
	palmas_vbus_irq(PALMAS_VBUS_IRQ, palmas_usb);

	if (palmas_usb->linkstat == USB_EVENT_NONE)
		palmas_id_irq(PALMAS_ID_IRQ, palmas_usb);
...
static int __devinit palmas_usb_probe(struct platform_device *pdev)
{
	...
	/* Need the charger ready to receive VBUS status before we probe */
#if defined(CONFIG_CHARGER_PALMAS) || defined(CONFIG_CHARGER_PALMAS_MODULE)
	if (is_palmas_charger(palmas->id) && !palmas->charger)
		return -EPROBE_DEFER;
#endif
	...
	if (!pdata->no_control_vbus) {
		palmas_usb->vbus_reg = regulator_get(palmas->dev, "vbus");
	...
	status = palmas_request_irq(palmas, PALMAS_ID_IRQ,
				"palmas_usb", palmas_id_irq, palmas_usb);
	if (status < 0) {
		dev_err(&pdev->dev, "can't get IRQ %d, err %d\n",
				PALMAS_ID_IRQ, status);
		goto fail_irq1;
	}

	status = palmas_request_irq(palmas, PALMAS_VBUS_IRQ,
				"palmas_usb", palmas_vbus_irq, palmas_usb);
	if (status < 0) {
		dev_err(&pdev->dev, "can't get IRQ %d, err %d\n",
				PALMAS_VBUS_IRQ, status);
		goto fail_irq2;
	}
...
fail_irq2:
	palmas_free_irq(palmas, PALMAS_ID_IRQ, palmas_usb);
...
static int __devexit palmas_usb_remove(struct platform_device *pdev)
{
	...
	palmas_free_irq(palmas, PALMAS_ID_IRQ, palmas_usb);
	palmas_free_irq(palmas, PALMAS_VBUS_IRQ, palmas_usb);
