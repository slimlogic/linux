...
static irqreturn_t tps65910_irq(int irq, void *irq_data)
{
	...
	tps65910->read(tps65910, TPS6591X_INT_STS, 1, &reg);
	irq_sts = reg;
	tps65910->read(tps65910, TPS6591X_INT_STS2, 1, &reg);
	irq_sts |= reg << 8;

	irq_mask = tps65910->read(tps65910, TPS6591X_INT_MSK, 1, &reg);
	irq_mask = reg;
	tps65910->read(tps65910, TPS6591X_INT_STS2, 1, &reg);
	irq_mask |= reg << 8;
	...
	reg = irq_sts & 0xFF;
	tps65910->write(tps65910, TPS6591X_INT_STS, 1, &reg);
	reg = irq_sts >> 8;
	tps65910->write(tps65910, TPS6591X_INT_STS2, 1, &reg);
...
static void tps65910_irq_sync_unlock(unsigned int irq)
{
	...
	tps65910->read(tps65910, TPS6591X_INT_MSK, 1, &reg);
	reg_mask = reg;
	tps65910->read(tps65910, TPS6591X_INT_MSK2, 1, &reg);
	reg_mask |= reg << 8;

	if (tps65910->irq_mask != reg_mask) {
		reg = tps65910->irq_mask & 0xFF;
		tps65910->write(tps65910, TPS6591X_INT_MSK, 1, &reg);
		reg = tps65910->irq_mask >> 8;
		tps65910->write(tps65910, TPS6591X_INT_MSK2, 1, &reg);
	}
		    ...
		    struct tps65910_platform_data *pdata)
{
	...
	for (cur_irq = tps65910->irq_base;
	     cur_irq < TPS6591X_NUM_IRQ + tps65910->irq_base;
	     cur_irq++) {
