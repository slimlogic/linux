		...
		unsigned int data)
{
	...
	return regmap_write(palmas->regmap[RTC_SLAVE], addr, data);
}
		...
		u8 *dest, size_t count)
{
	...
	return regmap_bulk_read(palmas->regmap[RTC_SLAVE], addr, dest, count);
}
		...
		u8 *src, size_t count)
{
	...
	return regmap_raw_write(palmas->regmap[RTC_SLAVE], addr, src, count);
}
		...
		unsigned int data)
{
	...
	return regmap_update_bits(palmas->regmap[RTC_SLAVE], addr, data, data);
}
		...
		unsigned int data)
{
	...
	return regmap_update_bits(palmas->regmap[RTC_SLAVE], addr, data, 0);
}
...
static int __devinit palmas_rtc_probe(struct platform_device *pdev)
{
	...
	ret = palmas_request_irq(palmas, PALMAS_RTC_ALARM_IRQ,
				"palmas_rtc", palmas_rtc_interrupt, palmas_rtc);
	if (ret < 0) {
		dev_err(&pdev->dev, "can't get IRQ %d, err %d\n",
				PALMAS_RTC_ALARM_IRQ, ret);
		goto out3;
	}
...
static int __devexit palmas_rtc_remove(struct platform_device *pdev)
{
	...
	palmas_free_irq(palmas, PALMAS_RTC_ALARM_IRQ, palmas_rtc);
	rtc_device_unregister(rtc);
	kfree(palmas_rtc);
