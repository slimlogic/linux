		...
		unsigned int *dest)
{
	...
	return regmap_read(leds->palmas->regmap[LEDS_SLAVE], addr, dest);
}
		...
		unsigned int value)
{
	...
	return regmap_write(leds->palmas->regmap[LEDS_SLAVE], addr, value);
}

static int palmas_led_update_bits(struct palmas_leds_data *leds, unsigned int reg,
		unsigned int mask, unsigned int value)
{
	...
	return regmap_update_bits(leds->palmas->regmap[LEDS_SLAVE], addr, mask,
					value);
}
...
static void palmas_leds_work(struct work_struct *work)
{
	...
	if (is_palmas_charger(led->leds_data->palmas->id)) {
...
static int palmas_leds_probe(struct platform_device *pdev)
{
	...
	if (!palmas->led_muxed && !is_palmas_charger(palmas->id)) {
