...
static int __devinit palmas_pwron_probe(struct platform_device *pdev)
{
	...
	err = palmas_request_irq(palmas, PALMAS_PWRON_IRQ,
			"palmas_pwron", pwron_irq, pwron);
	if (err < 0) {
...
free_irq:
	palmas_free_irq(palmas, PALMAS_PWRON_IRQ, pwron);
free_input_dev:
...
static int __devexit palmas_pwron_remove(struct platform_device *pdev)
{
	...
	struct palmas *palmas = pwron->palmas;
